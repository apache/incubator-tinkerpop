...
model for embedded transactions. Migrating to the new model would be preferred at this point, but nothing (with the
exception of the `Transaction#createdThreadedTransaction()` method) has been officially deprecated as the API itself
is technically still relevant.
